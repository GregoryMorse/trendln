...
def calc_support_resistance(h, extmethod = METHOD_NUMDIFF, method=METHOD_NSQUREDLOGN, window=125, errpct = 0.005, hough_scale=0.01, hough_prob_iter=10, sortError=False):
    ...
    def hough(Idxs):
        ...
        for pts, angle, dist in zip(*hough_line_peaks(h, theta, d, threshold=2)): #> threshold
            y0, y1 = (dist - origin * np.cos(angle)) / np.sin(angle)
            y0, y1 = y0 / scl + m, y1 / scl + m
    ...
    def measure_area(trendline, isMin): # Reimann sum of line to discrete time series data
        ...
        base = trendline[0][0]
        m, b, ser = trendline[1][0], trendline[1][1], h[base:trendline[0][-1]+1] #error with support lines???
